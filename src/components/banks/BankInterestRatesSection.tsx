...
export function BankInterestRatesSection({ page }: { page: BankSeoPageData }) {
  ...
  const rates = (page.interestRates && page.interestRates.length > 0)
    ? [...page.interestRates].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0))
    : Array(4).fill({
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {(products.length > 0 ? products : [
              { title: "Personal Loan", description: "Loan up to ₹40 Lakh\nInterest from 10.50% p.a." },
              { title: "Home Loan", description: "Loan up to ₹10 Cr\nInterest from 8.40% p.a." },
              { title: "Business Loan", description: "Loan up to ₹1 Cr\nInterest from 11.25% p.a." },
              { title: "Credit Card", description: "Lifetime Free Cards\nExclusive Rewards" }
            ]).map((product, index) => {
        ...
        <div className="mx-auto max-w-7xl rounded-2xl border border-[#e3ebf3] bg-white p-6 md:p-8 shadow-[0_4px_16px_rgba(22,34,50,0.02)] grid gap-6 lg:grid-cols-[1fr_280px]">
          ...
          <div className="space-y-4">
            <h2 className="text-[20px] font-bold text-[#000000] tracking-tight">
              {page.bankName || "HDFC"} {page.productName || "Personal Loan"} Interest Rates
            </h2>
              ...
              <table className="w-full min-w-140 border-collapse text-left text-[13px]">
                <thead>
                  <tr className="border-b border-[#e3ebf3]">
                    {["Loan Amount", "Interest Rate (p.a.)", "Processing Fee", "Loan Tenure"].map((heading) => (
                      <th key={heading} className="px-4 py-3.5 font-bold text-[#2d3142] bg-[#fdfeff]">
                        {heading}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody className="divide-y divide-[#eef2f6]">
                  {rates.map((rate, index) => (
                    <tr key={index} className="hover:bg-[#fafcfe]">
                      <td className="px-4 py-4 font-medium text-[#93989f]">{rate.loanAmount}</td>
                      <td className="px-4 py-4 font-medium text-[#93989f]">{rate.interestRate}</td>
                      <td className="px-4 py-4 font-medium text-[#93989f]">{rate.processingFee}</td>
                      <td className="px-4 py-4 font-medium text-[#93989f]">{rate.tenure}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-[12px] font-medium leading-relaxed text-[#7a869a]">
                  Get the best loan offer from {page.bankName || "HDFC Bank"} in just a few minutes.
                </p>
              </div>

              {/* Checklist Lines Component */}
              <div className="space-y-3">
                {(page.applyBullets && page.applyBullets.length > 0 ? page.applyBullets : [
                  "Minimal Documentation",
                  "Quick Disbursal"
                ]).map((item) => (
                  <div key={item} className="flex items-center gap-2.5 text-[13px] font-medium text-[#7a869a]">
