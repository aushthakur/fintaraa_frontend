...
import type { BankSeoLocationPage } from "@/services/bankSeoPages";
...
const makeTitle = (
  ...
  const suffix = [
    ...
    .join(", ");
  return suffix
    ? `${bankName} ${productName} in ${suffix}`
    : `${bankName} ${productName}`;
};

const buildBankDirectoryPath = (
  bankSlug: string,
  productSlug: string,
  location?: Partial<ParsedLoanLocation>,
) =>
...
export function BankLocationDirectory({
  bankName,
  bankSlug,
  productName,
  productSlug,
  ...
  const titleName = `${bankName} ${productName}`;
  const stateItems = uniqueItems(
    locations.map((page) => ({
      key: `state:${page.location.state}`,
      label: makeTitle(bankName, productName, { state: page.location.state }),
      href: buildBankDirectoryPath(bankSlug, productSlug, {
  ...
  const cityItems = uniqueItems(
    currentStatePages
      ...
      .map((page) => ({
        ...
        label: makeTitle(bankName, productName, {
          state: page.location.state,
  ...
  const areaItems = uniqueItems(
    areaSource
      ...
      .map((page) => ({
        ...
        label: makeTitle(bankName, productName, page.location),
        href:
          page.canonicalPath ||
          buildBankDirectoryPath(bankSlug, productSlug, {
            state: page.location.state,
            city: page.location.city,
            pincode: page.location.pincode,
    ...
    <section className="bg-[#fbfcfd] px-4 py-8 md:px-6 lg:px-8">
      <div className="mx-auto max-w-9xl divide-y divide-[#edf1f4]">
        {!hasState ? (
          <>
            <DirectorySection
              title={`${titleName} Pages By State`}
              icon={Map}
              items={stateItems}
              maxItems={60}
            />
            <DirectorySection
              title={`Popular ${titleName} City Pages`}
              icon={Building2}
              items={cityItems}
              maxItems={90}
            />
            <DirectorySection
              title={`Popular ${titleName} Area Pages`}
              icon={MapPin}
              items={areaItems}
              maxItems={100}
            />
          ...
          <>
            <DirectorySection
              title={`${titleName} Pages In ${current.state}`}
              icon={Navigation}
            ...
            <DirectorySection
              title={
                hasCity
                  ? `${titleName} Pincode Pages In ${current.city}`
                  : `${titleName} Pincode Pages In ${current.state}`
              }
              icon={LocateFixed}
              items={pincodeItems}
              maxItems={140}
            ...
            <DirectorySection
              title={
                hasPincode
                  ? `${titleName} Area Pages In ${current.pincode}`
                  : hasCity
                    ? `${titleName} Area Pages In ${current.city}`
                    : `${titleName} Area Pages In ${current.state}`
              }
              icon={MapPin}
              items={areaItems}
              maxItems={150}
            ...
            {hasArea ? (
              <DirectorySection
                title={`More ${bankName} Bank Product Pages`}
                icon={CreditCard}
                items={stateItems}
                maxItems={60}
              />
