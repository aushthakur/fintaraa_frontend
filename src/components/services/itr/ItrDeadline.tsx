  ...
  return (
    <section className="px-4 py-8 md:px-6 lg:px-8">
      <div className="mx-auto max-w-9xl rounded-xl border border-[#d7dfe8] bg-white p-5 sm:p-6 md:p-8">
        <h2 className="text-xl font-bold text-[#1a6bc6] sm:text-2xl md:text-[26px] lg:text-[28px]">
          Key Deadline
        </h2>

        <div className="mt-6 grid gap-6 divide-y divide-[#e3e8ee] sm:mt-8 md:grid-cols-3 md:divide-x md:divide-y-0">
          {deadlines.map(({ day, suffix, rest, label, iconSrc, iconAlt }, index) => (
            <div
              key={label}
              className={`text-center ${index > 0 ? "pt-6 md:pt-0 md:pl-6" : ""}`}
            >
              <h3 className="text-base font-bold text-[#111827] sm:text-[17px] md:text-[19px] lg:text-xl">
                {day}
