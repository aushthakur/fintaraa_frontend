      ...
      <div className="mx-auto max-w-9xl">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {dsaBenefits.map(({ title, text, icon: Icon }) => (
            <div
              key={title}
              className="flex items-center gap-4 rounded-xl border border-[#d9e2ec] bg-white p-4"
            >
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded bg-[#e8f4ff] text-[#005ca8]">
          ...
          <div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-4 md:grid-cols-4">
              {eligibleProfiles.map(({ title, icon: Icon }) => (
                <div
                  key={title}
                  className="rounded border border-[#d9e2ec] bg-white p-5 text-center"
                >
                  <span className="mx-auto flex h-10 w-10 items-center justify-center rounded bg-[#e8f4ff] text-[#005ca8]">
                    <Icon className="h-5 w-5" />
                  </span>
                  <p className="mt-4 text-[12px] font-black text-[#111827]">
                    {title}
                  </p>
                </div>
              ))}
        ...
        <div
          id="dsa-form"
          className="mt-8 grid gap-6 rounded-lg border border-[#d9e2ec] bg-white p-5 shadow-[0_12px_35px_rgba(16,24,40,0.06)] lg:grid-cols-[1fr_23rem]"
        >
          <div>
            <h2 className="text-[22px] font-black text-[#111827]">
            ...
            <form className="mt-5 grid gap-4 sm:grid-cols-2">
              {formFields.map(([label, placeholder, type]) => (
                <label key={label} className="grid gap-1">
                  ...
                  {type === "select" ? (
                    <select className="h-10 rounded border border-[#d7dfe9] bg-white px-3 text-[12px] font-semibold text-[#667085] outline-none">
                    ...
                    <input
                      ...
                      className="h-10 rounded border border-[#d7dfe9] px-3 text-[12px] font-semibold outline-none placeholder:text-[#98a2b3]"
                    />
                  )}
                </label>
              ))}
              <button
                type="button"
                className="h-12 rounded-full bg-[#13a653] text-[13px] font-black text-white sm:col-span-2"
              >
                Become a Partner
              </button>
          ...
          <aside className="rounded-lg bg-[#e8f4ff] p-5">
            <h3 className="text-[20px] font-black text-[#111827]">
              Benefits You Get
            </h3>
