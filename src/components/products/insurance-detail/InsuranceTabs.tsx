...
function InsuranceTabIcon({
  ...
  if (
    identity.includes("premium") ||
    identity.includes("plan") ||
    identity.includes("compare")
  )
  ...
  if (identity.includes("product") || identity.includes("other"))
    return <LayoutGrid className={className} />;
  if (identity.includes("coverage") || identity.includes("cover"))
...
const shortLabel = (label: string) => {
  ...
  if (value.includes("compare")) return "Plans";
  if (value.includes("document")) return "Documents";
  if (value.includes("eligib")) return "Eligibility";
  if (value.includes("question")) return "FAQs";
  return label;
};

const guideTitle = (title: string) =>
  title.replace(
    /\b(coverage|eligibility|documents|claims|premium|plans|reviews|faqs)\b/gi,
    (word) =>
...
export function InsuranceTabs({
  ...
  return (
    ...
    >
      <div
        role="tablist"
        aria-label="Insurance information sections"
...
export function InsuranceGuidePanel({
  tabs,
  activeTab,
  onTabChange,
  productName,
    ...
    <section className="bg-[#f7fafc] pb-10 md:pb-14">
      <div className="mx-auto grid max-w-9xl items-start gap-5 px-4 pt-7 sm:px-6 md:pt-9 lg:grid-cols-[245px_minmax(0,1fr)] lg:px-8">
        ...
        >
          ...
          <nav
            aria-label={`${productName} guide sections`}
        ...
        <article className="min-w-0 rounded-2xl border border-[#dce7ef] bg-white p-5 sm:p-7 md:p-8">
          ...
          <div className="mt-7 border-t border-[#e5edf3] pt-5">
            <p className="text-[10px] font-medium leading-5 text-[#8295a5]">
              Coverage, premiums, eligibility, exclusions and final document
              requirements depend on the selected insurer, policy wording and
