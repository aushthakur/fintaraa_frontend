...
const catalogPath = path.join(
  projectRoot,
  "src/data/insuranceBannerCatalog.json",
);
const sourceDirectory = path.join(
  projectRoot,
  "public/assets/insurance-banners/source",
);
const outputDirectory = path.join(
  projectRoot,
  "public/assets/insurance-banners/rendered",
);
const catalog = JSON.parse(await readFile(catalogPath, "utf8"));

const accents = {
  "life-protection": "#53d6cb",
  "health-protection": "#59c8ff",
  "retirement-planning": "#65d3c6",
  "motor-protection": "#4cc8ff",
  "travel-protection": "#62bfff",
  "property-protection": "#5ed2be",
  "business-protection": "#4fd4a5",
  "machinery-protection": "#63b9ff",
  "assisted-insurance": "#55d6ca",
};

const sourceImage = (family) =>
  path.join(sourceDirectory, `${family}.png`);
...
const desktopOverlay = (profile, slide) => {
  ...
  const accent =
    accents[isAssisted ? "assisted-insurance" : profile.family] || "#55d6ca";
  const eyebrow = isAssisted
    ? `ASSISTED ${profile.name.toUpperCase()} JOURNEY`
    : profile.eyebrow;
  const title = isAssisted
    ? `Choose ${profile.name} with Expert Guidance`
    : profile.title;
  ...
  return Buffer.from(`<svg width="1600" height="640" viewBox="0 0 1600 640" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="leftShade" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0" stop-color="#06152d" stop-opacity="0.95"/>
...
const createDesktopBanner = async (profile, slide) => {
  const family = slide === 1 ? profile.family : "assisted-insurance";
  const outputPath = path.join(
    outputDirectory,
    `${profile.slug}-${String(slide).padStart(2, "0")}-desktop.webp`,
  );
  await sharp(sourceImage(family))
...
console.log(
  `Generated ${manifest.length} insurance banners with desktop and mobile art direction.`,
);
